...
if (cc_comu_requereix_asrai3 != 2) then {
    if (cc_comu_requereix_asrai3 != cc_param_asrai3_habilitat) then {
        if (cc_param_asrai3_habilitat == 1) then {
            player sideChat format ["ERROR (cc_comu_comprovarConfiguracio_asrai3_asrai3.sqf): El mod ASR AI 3 es necessari per aquesta missió però no està carregat."];
        } else {
        ...
        if ((cc_param_asrai3_habilitat == 1) and !cc_mod_asrai3) then {
            player sideChat format ["ERROR (cc_comu_comprovarConfiguracio_asrai3.sqf): El mod ASR AI 3 es necessari per aquesta missió però el fitxer asr_ai3_main.pbo no està carregat."];
        };
    };
} else {
    if ((cc_param_asrai3_habilitat == 1) and !cc_mod_asrai3) then {
