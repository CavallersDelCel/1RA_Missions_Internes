...
if (isServer) then {
    asr_ai3_main_enabled            = cc_param_asrai3_habilitat;        // All the other settings matter only if we have 1 here; set 0 to disable all scripted features
...
//    asr_ai3_main_dayscope           = 1;        // Prevent AI from using scope fire modes at night (reduced engagement range and accuracy);
    ...
    if ( cc_param_debugOutput == 1 ) then {
        player sideChat format ["DEBUG (fn_asrai_config.sqf): ASR AI configurat."];
    };
};
