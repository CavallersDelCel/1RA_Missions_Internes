...
cc_comu_requereix_dac         = 2;  // Variable per controlar l'ús del Dynamic-AI-Creator (DAC).
cc_comu_requereix_perfils1RA  = 2;  // Variable per controlar l'ús dels perfils de Virtual Arsenal del grup 1RA.
cc_comu_requereix_t8units     = 2;  // Variable per controlar l'ús del T8 Units.
...
if (isClass(missionconfigfile>>"CfgComponents">>"t8units")) then {
    if (cc_comu_requereix_t8units == 0) then {
        player sideChat format ["ERROR: L'addon del T8 Units està carregat però no és necessari per aquesta missió."];
    } else {
        #include "includes\cc_comu_comprovarConfiguracio_t8units.sqf"
    };
} else {
    if (cc_comu_requereix_t8units == 1) then {
        player sideChat format ["ERROR: L'addon del T8 Units no està carregat però és necessari per aquesta missió."];
    };
};
