...
if (isClass (configFile >> "CfgPatches" >> "ace_maptools")) then {
    ["ace_maptools_EveryoneCanDrawOnBriefing", true, true, true] call ACE_common_fnc_setSetting;               // 0* = Disabled, 1 = Enabled.
} else {
    _moduleError = true;
    diag_log "(BMT Debug) File not found: ace_maptools.pbo.";
};
...
if (isClass (configFile >> "CfgPatches" >> "ace_medical")) then {
    ...
    ["ace_medical_useCondition_SurgicalKit", 1.0, true, true] call ACE_common_fnc_setSetting;                  // 0* = Anytime, 1 = Stable.
