/*
V2.3 Script by: Ghost (Original dialog from Kronzky) create a marker and name itand put this in the object initline - ghst_spawnveh = station1 addAction ["Spawn Vehicle", "ghst_spawnveh.sqf", ["spawnmarker",dir], 6, true, true, "","alive _target"];
*/
...
DLG_VEH_SELECTED = false;
	...
	if (count _padempty > 0) exitwith {hint "Spawn Pad not clear";};
	_veh1 = createVehicle [_vehsel,_spawn, [], 0, "NONE"];
	_veh1 setdir _dir;
	_veh1 setposatl [_spawn select 0, _spawn select 1, 0.2];
	...
	if ((_PARAM_NIMITZ == 1) and ((_vehsel == "JS_JC_FA18E") or (_vehsel == "JS_JC_FA18F"))) then {
		NIMCAT_GAMELOGIC synchronizeObjectsAdd [_veh1];
		[_veh1,NIMCAT_GAMELOGIC] spawn TTT_fnc_syncCatapult;
	};
