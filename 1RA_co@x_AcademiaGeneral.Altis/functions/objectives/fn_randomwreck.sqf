...
_posatl = [_position_mark,[_wreckrad,_wreckrad,(random 360)],true] call fnc_ghst_rand_position;
...
//_wreck_name = getText (configFile >> "cfgVehicles" >> (_wreckobj) >> "displayName");
	...
	for "_b" from 0 to ceil(random _patrol_group)+1 do {
		...
		_posatl = [_position_mark,_patrol_rad,true] call fnc_ghst_rand_position;
...
if (floor (random 10) > 5) then {
	...
	_taskdesc = format ["Search %2 for %1. Pay attention to the wrecks.", _veh_name, _locselname];
	...
	_taskdesc = format ["Locate and destroy %1 found around %2. Pay attention to the wrecks.", _veh_name, _locselname];
	[_tsk,_tasktopic,_taskdesc,true,[],"created",_position_mark] call SHK_Taskmaster_add;
