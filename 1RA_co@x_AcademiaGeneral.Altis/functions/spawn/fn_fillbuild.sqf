...
example
null0 = [[1,2,3],300,EAST,[80,40],[markunits (true/false),"color"],aiskill 0.4,debug(true/false),pubvar(true/false)] execvm "ghst_fillbuild3.sqf";
example
null0 = [[1,2,3],200,EAST,[30,15],[true,"ColorRed"],0.4,true,true] execvm "ghst_fillbuild3.sqf";
*/
...

{
	...
	while { ! _pIsEmpty } do 
	{
		...
		if ((str _p != "[0,0,0]") and !(_x iskindof "Piers_base_F") and !(isNil "_p")) then
		{
			_b = _b + [_p];
			_all_positions = _all_positions + [_p];
		}
		else
		{
			_pIsEmpty = true;
...
_sumpos = floor(random count _all_positions);
if (_sumpos > _maxnum) then {_sumpos = (_maxnum - floor(random _minnum));};
//if (_sumpos < _minnum) then {_sumpos = _sumpos;};

if (debug) then {diag_log format ["Max pos: %1, Unit Side: %2, Number of pos: %3, BuildPos array: %4", _sumpos, _sideguards, count _all_positions, _build_positions];};
...
for "_p" from 0 to (_sumpos) do {
	...
	_mansel = _menlist call BIS_fnc_selectRandom;
		...
		if (!(isnil "_positionarray") and (floor (random 10) > 7)) then {
			private ["_r","_position"];
			...
			if (_markunits) then {
			_mark1 = [_position,_mcolor,"","mil_objective"] call fnc_ghst_mark_point;
			};
			// spawn patrol script
			...
			if (_markunits) then {
			_mark1 = [_position,_mcolor] call fnc_ghst_mark_point;
			};
		};
	//sleep 0.03;
};
