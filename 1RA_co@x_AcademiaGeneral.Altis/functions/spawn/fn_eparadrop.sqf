...
_paralist = ghst_menlist;
_ghst_side = ghst_side;
_flyheight = 200;

_parapos = [_objpos,_radarray] call fnc_ghst_rand_position;

//create marker
if (_markunits) then {
	[_parapos,_mcolor,"Transport Waypoint UNLOAD"] call fnc_ghst_mark_point;
};
...
//create marker
if (_markunits) then {
	[_spawnpos,_mcolor,"Transport Spawn"] call fnc_ghst_mark_point;
};


_dir = [_spawnpos, _objpos] call BIS_fnc_dirTo;
...
_cargoPositions = _air1 emptyPositions "cargo";
	for "_x" from 0 to (_cargoPositions)-1 do {
		...
		_mansel = _paralist call BIS_fnc_selectRandom;
		_man = [_spawnpos,_paras1,_mansel,aiSkill] call fnc_ghst_create_unit;
...
[_paras1,_objpos, _radarray,_markunitsarray,["COMBAT", "FULL", "WEDGE"]] call fnc_ghst_waypoints;
	...
	While {(alive _air1) and (canMove _air1)} do {
		...
		Waituntil {(_air1 distance _parapos) < (_flyheight + 100)};

			{
				...
				(_x) action ["EJECT", vehicle _air1];
				sleep 0.4;
				_chute = createVehicle ["NonSteerable_Parachute_F",(getPos _x), [], 0, "FLY"];
