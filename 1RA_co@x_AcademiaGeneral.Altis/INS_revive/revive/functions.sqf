...
INS_REV_FNCT_moveInVehicle = { 
    private ["_id","_unit","_vehicle","_turrets","_sub_turrets","_position"];
...
INS_REV_FNCT_respawn_tag = {
	...
	if (!(INS_REV_GVAR_camPlayer call INS_REV_FNCT_isRespawnLocation)) then {
		...
		if (INS_REV_GVAR_camPlayer iskindof "AllVehicles") then {
			_name = toUpper (format["%1",(getText (configFile >> "cfgVehicles" >> (typeOf INS_REV_GVAR_camPlayer) >> "displayName"))]);//vehiclevarname
		} else {
			_name = toUpper (format["%1",vehiclevarname INS_REV_GVAR_camPlayer]);
		};
	};
	...
	while {_condition && alive _unit} do {
		...
		if (_doRespawn) exitWith {
			...
			if (call INS_REV_FNCT_can_respawn) then {
				if ([INS_REV_GVAR_camPlayer] call INS_REV_FNCT_is_vehicle && !(vehicle INS_REV_GVAR_camPlayer call INS_REV_FNCT_vclisFull) and getposatl INS_REV_GVAR_camPlayer select 2 > 5) then {
					...
					_position = (getPosATL INS_REV_GVAR_camPlayer) findEmptyPosition [0,15,typeof _unit];
					_unit setDir getDir INS_REV_GVAR_camPlayer;
					_unit setPosATL [_position select 0, _position select 1, 0];
...
INS_REV_FNCT_onKilled_process = {
	...
	// Revert Loadout
	if (GVAR_is_arma3) then {
		//[_player, [missionnamespace, "Current"]] call bis_fnc_loadInventory;
