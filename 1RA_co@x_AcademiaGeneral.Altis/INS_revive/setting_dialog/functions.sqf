FNC_check_display = {
	if (INS_REV_CFG_taw_view) then {
		...
		systemChat "La configuració de pantalla no es disponible.";
	};
};
...
FNC_check_squad = {
	if (INS_REV_CFG_squad_management) then {
		...
		systemChat "La gestió de jugadors no es disponible.";
	};
};
...
FNC_check_uav = {
	if (INS_REV_CFG_uav && INS_REV_CFG_uav_setting_dialog) then {
		if (GVAR_is_arma3) then {
			...
			systemChat "La configuració de UAV no es disponible.";
		};
	} else {
		systemChat "La configuració de UAV no es disponible.";
	};
