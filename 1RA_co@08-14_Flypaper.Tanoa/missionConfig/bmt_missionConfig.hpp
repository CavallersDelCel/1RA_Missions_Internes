...
overviewText = "Russian presence on Tanoa is reaching highly threating levels. Fortunately their bases of operations are still in construction, which opens a window for sabotaging their must importnt assets. A small group of marines are deployed in Tanoa with the task of sabotaging their newest air base. Stealth is essencial: despite the base lacking static defenses, there is a strong presence of enemy patrols.";          // Text shown when selecting mission.
