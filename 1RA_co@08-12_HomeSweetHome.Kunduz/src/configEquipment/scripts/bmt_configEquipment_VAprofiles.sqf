...
switch (_unitRole) do
{
    // Squad Leader.
    case "sl": {
        _VAProfile = "CC - Lider d'esquadra M4A1+M320 OCP";
        _loadout = [_unit,[profileNamespace,_VAProfile]] call BIS_fnc_loadInventory;
    };

    // Team leader.
    case "tl": {
        _VAProfile = "CC - Lider d'equip M4A1+M320 OCP";
        _loadout = [_unit,[profileNamespace,_VAProfile]] call BIS_fnc_loadInventory;
    };

    // Rifleman.
    case "rfl": {
        _VAProfile = "CC - Fuseller M4A1 OCP";
        _loadout = [_unit,[profileNamespace,_VAProfile]] call BIS_fnc_loadInventory;
    };

    // Rifleman AT4.
    case "rflat": {
        _VAProfile = "CC - AT M4A1+AT4 OCP";
        _loadout = [_unit,[profileNamespace,_VAProfile]] call BIS_fnc_loadInventory;
    };

    // Grenadier.
    case "gr": {
        _VAProfile = "CC - Granader M4A1+M320 OCP";
        _loadout = [_unit,[profileNamespace,_VAProfile]] call BIS_fnc_loadInventory;
    };

    // Automatic Rifleman.
    case "ar": {
        _VAProfile = "CC - FA M249 OCP";
        _loadout = [_unit,[profileNamespace,_VAProfile]] call BIS_fnc_loadInventory;
    };

    // Machine Gunner.
    case "mg": {
        _VAProfile = "CC - Metrallador M240B OCP";
        _loadout = [_unit,[profileNamespace,_VAProfile]] call BIS_fnc_loadInventory;
    };

    // Anti-Tank.
    case "at": {
        _VAProfile = "CC - AT M4A1+Javelin OCP";
        _loadout = [_unit,[profileNamespace,_VAProfile]] call BIS_fnc_loadInventory;
    };

    // (Anti-Aircraft Gunner.
    case "aa": {
        _VAProfile = "CC - AT M4A1+Stinger OCP";
        _loadout = [_unit,[profileNamespace,_VAProfile]] call BIS_fnc_loadInventory;
    };

    // Designated Marksman.
    case "dm": {
        _VAProfile = "CC - Tirador SR-25EC OCP";
        _loadout = [_unit,[profileNamespace,_VAProfile]] call BIS_fnc_loadInventory;
    };

    // Platoon Medic.
    case "me": {
        _VAProfile = "CC - Metge M4A1 OCP";
        _loadout = [_unit,[profileNamespace,_VAProfile]] call BIS_fnc_loadInventory;
    };

    // Explosives Specialist.
    case "exp": {
        _VAProfile = "CC - Explosius M4A1 OCP";
        _loadout = [_unit,[profileNamespace,_VAProfile]] call BIS_fnc_loadInventory;
    };

    // Sniper.
    case "sn": {
        _VAProfile = "CC - Franctirador M2010ESR OCP";
        _loadout = [_unit,[profileNamespace,_VAProfile]] call BIS_fnc_loadInventory;
    };

    // Spotter.
    case "sp": {
        _VAProfile = "CC - Observador M4A1 OCP";
        _loadout = [_unit,[profileNamespace,_VAProfile]] call BIS_fnc_loadInventory;
    };

    // Diver Squad Leader.
    case "divsl": {
        _VAProfile = "CC - Bussejador lider M4A1";
        _loadout = [_unit,[profileNamespace,_VAProfile]] call BIS_fnc_loadInventory;
    };

    // Diver Medic.
    case "divme": {
        _VAProfile = "CC - Bussejador metge M4A1";
        _loadout = [_unit,[profileNamespace,_VAProfile]] call BIS_fnc_loadInventory;
    };

    // Diver Explosives Specialist.
    case "divexp": {
        _VAProfile = "CC - Bussejador explosius M4A1";
        _loadout = [_unit,[profileNamespace,_VAProfile]] call BIS_fnc_loadInventory;
    };

    // Diver.
    case "div": {
        _VAProfile = "CC - Bussejador M4A1";
        _loadout = [_unit,[profileNamespace,_VAProfile]] call BIS_fnc_loadInventory;
    };
